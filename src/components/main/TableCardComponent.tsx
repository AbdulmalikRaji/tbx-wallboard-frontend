  ...
  try {
    return (
      <div className="flex flex-col sm:flex-row items-center space-y-2 sm:space-y-0 sm:space-x-4  m-3 p-3 rounded-xl bg-slate-100 drop-shadow-md">
        ...
        <div>
          <h4 className="text-gray-700 text-sm sm:text-lg md:text-xl font-semibold">{product.ProductName}</h4>
          <h6 className="text-gray-700 text-xs sm:text-sm">{product.Category}</h6>
          <p className="text-gray-500 line-clamp-1 text-xs sm:text-sm">Barcode: {product.Barcode}</p>
          <p className="text-gray-500 block text-xs sm:text-sm">Shop: {product.Shop}</p>
          <p className="text-gray-500 text-xs sm:text-sm">Location: {product.Location[0]}, {product.Location[1]}</p>
        </div>
      </div>
    );
  } catch (err) {
